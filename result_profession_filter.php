...
<?php 
	...
	$query = "SELECT profession FROM PROFESSIONS WHERE id = $profession LIMIT 1";
	$result = mysqli_query($connection, $query);

	if (!$result) {
		die("Database query failed");
	} 
		$profession_name = mysqli_fetch_object($result);
?>

<?php 
	
	$query = "SELECT Count(*) as count FROM MEMBERS WHERE profession = $profession";
	...
	if (!$result) {
		...
		$profession_count = mysqli_fetch_object($result);
?>

		<?php 
			$query = "SELECT m.id as id, m.first_name as first_name, m.middle_initial as middle_initial, m.last_name as last_name, m.added_by, m.profession,
					a.id  as added_id, a.first_name as added_first, a.middle_initial as added_middle, a.last_name as added_last, DATE_FORMAT(m.joined_date, '%c/%e/%y') AS 'Date Joined', p.profession
					FROM MEMBERS m
...
<?php
 ...
 <div class="main">
	<div class="grid_details">
		<h2><?php echo $profession_name->profession . " matches "; ?>
			<?php if ( $profession_count->count == '1'){
						echo $profession_count->count . " Member" ;
					} else {
						echo $profession_count->count  .  " Members" ;
					}
			  ?>
		</h2>
